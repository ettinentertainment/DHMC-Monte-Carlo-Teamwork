function runSimulation(count, mapSize) {

    //Log start of simulation to F12 console
    console.log("DEBUG: runSimulation started with map size:", mapSize);
    ...
    const gameState = {
        ...
        addD4Damage: false // New
    };

    // Check for BP Modifiers
    ...
    //New: Save Survivors from Single Simulations
    if(count===1) {
        survivorPool = []; //Clear the pool for a new run
        const survivors = gameState.players.filter(p => p.current_hp > 0);
        if(survivors.length > 0) {
            simLog(` --- Saving ${survivors.length} Survivor(s) ---`);
            survivors.forEach(survivor => { 
                //Ensure the survivor has a unique ID for the pool
    ...
    for (const card of player.domainCards) {
        switch (card.name) {
            ...
            case "Bolt Beacon":
                ...
                break;
            case "Book Of Illiat":
                if (!player.abilityUsage['Arcane Barrage'] && isTargetInRange(player, target, "Close")) {
                    possibleActions.push({ type: 'SPELL', card: card, priority: 2, name: "Arcane Barrage" });
                } else if (isTargetInRange(player, target, "Very Close")) {
                    possibleActions.push({ type: 'SPELL', card: card, priority: 1, name: "Slumber" }); // Lower priority than barrage
                }
                break;
        }
    }
...
function executePCSpell(player, card, target, gameState) {
    ...
    switch (card.name) {
        ...
        case "Book Of Illiat":
            // This card has multiple spells. Prioritize Arcane Barrage if available.
            const arcaneBarrageSpell = { name: 'Arcane Barrage', range: 'Close' };
            const slumberSpell = { name: 'Slumber', range: 'Very Close' };
            let spellToCast = null;
            
            // Check for Arcane Barrage
            if (!player.abilityUsage['Arcane Barrage']) {
                if (isTargetInRange(player, target, arcaneBarrageSpell.range)) {
                    spellToCast = arcaneBarrageSpell;
                }
            } 
            
            // If Arcane Barrage wasn't viable, check for Slumber
            if (!spellToCast) {
                if (isTargetInRange(player, target, slumberSpell.range)) {
                    spellToCast = slumberSpell;
                }
            }
            
            if (!spellToCast) {
                simLog(` -> (Book of Illiat spells are out of range)`);
                return { outcome: 'FAILURE_WITH_FEAR' }; // Failsafe
            }
            
            // --- Execute the chosen spell ---
            simLog(` -> Casting \"${spellToCast.name}\" (from Book of Illiat) on ${target.name}!`);
            result = executeActionRoll(target.difficulty, traitMod, 0);
            simLog(` Roll: ${player.spellcastTrait} (${traitMod}) | Total ${result.total} vs Diff ${target.difficulty} (${result.outcome})`);
            
            if (result.outcome === 'CRITICAL_SUCCESS' || result.outcome === 'SUCCESS_WITH_HOPE' || result.outcome === 'SUCCESS_WITH_FEAR') {
                if (spellToCast.name === 'Arcane Barrage') {
                    // Mark as used
                    player.abilityUsage['Arcane Barrage'] = true;
                    simLog(` -> (Marking 'Arcane Barrage' as used for this rest)`);
                    // Per SRD, spend any number of Hope. Sim will spend 1 for now.
                    if (player.current_hope >= 1) {
                        player.current_hope--;
                        simLog(` -> Spent 1 Hope (Total: ${player.current_hope})`);
                        const damageTotal = rollDamage("1d6", 1, 0); // 1d6 per Hope spent
                        const damageInfo = { amount: damageTotal, isDirect: false, isPhysical: false, isStandardAttack: false };
                        applyDamage(damageInfo, player, target, gameState);
                    } else {
                        simLog(` -> (Not enough Hope to cast 'Arcane Barrage'!)`);
                    }
                } else if (spellToCast.name === 'Slumber') {
